...
**마지막 갱신**: 2026-09-07 (KOCCA·정부24 연동 및 운영 DB 적재 반영)
...
| 한국콘텐츠진흥원(KOCCA) | **연결됨** | 실제 API 호출 검증 완료, 운영 DB 4건 적재 완료 |
| 정부24 공공서비스(혜택) | **연결됨** | 소상공인 필터 실제 호출 검증 완료, 운영 DB 5건 적재 완료 |
...
- `programs` 총 **34건** = 기업마당 20건 + K-Startup 5건 + KOCCA 4건 + 정부24 5건
...
## 9월 7일 시작 체크리스트 (KOCCA·정부24 완료 후 남은 작업)
...
위 KOCCA 확인·실제 호출·운영 DB 적재는 2026-09-07에 완료되었다.

### 2-1. 정부24 공공서비스(혜택) 연동 (완료)

정부24 서비스List API의 실제 호출과 소상공인 필터 검증을 완료했고,
확인된 5건을 운영 DB에 적재했다. `data_sources` 등록과 목록·상세 화면의
출처 표시도 확인했다.
