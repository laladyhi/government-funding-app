...
Python 3.10 이상만 설치되어 있다면, 프로젝트 폴더의
**`앱_실행.cmd`를 더블클릭**하세요. 처음 실행할 때는 필요한 패키지 설치와
데이터베이스 생성을 자동으로 진행한 뒤 브라우저를 열어줍니다. 이후에는
더블클릭할 때마다 바로 서버가 켜지고 브라우저가 열립니다.

아래는 터미널에서 직접 실행하고 싶을 때의 수동 절차입니다.
